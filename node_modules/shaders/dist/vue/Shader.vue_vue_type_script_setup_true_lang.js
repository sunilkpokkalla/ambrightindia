...
import { rootPassthrough, shaderRendererGPU } from "../core/index.js";
...
var Shader_vue_vue_type_script_setup_true_lang_default = /* @__PURE__ */ defineComponent({
	...
	setup(__props, { expose: __expose, emit: __emit }) {
		...
		const startTelemetryWhenReady = () => {
			const checkRendering = () => {
				if (rendererInstance.value.getPerformanceStats().fps > 0) {
					telemetryCollector = startTelemetry(rendererInstance.value, "3.0.438", props.disableTelemetry, props.isPreview);
		...
		const setupVisibilityObserver = () => {
			...
			visibilityObserver = new IntersectionObserver((entries) => {
				...
				if (isCurrentlyVisible && !wasVisible) {
					...
					} else if (!externallyPaused) initializeRenderer().then(() => {
		...
		const beginRecordingResolution = async (pixelRatio) => {
			...
			const restoreResolution = rendererInstance.value.beginRecordingResolution(cssWidth, cssHeight, pixelRatio);
			await renderForCapture();
			await renderForCapture();
			return async () => {
				restoreResolution();
		...
		const renderSyntheticFrame = async (deltaSeconds) => {
			await rendererInstance.value.renderSyntheticFrame(deltaSeconds);
		};
