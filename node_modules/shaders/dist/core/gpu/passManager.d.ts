import { TgpuRoot, TgpuBindGroup } from 'typegpu';
import { CompositionIR } from './composer';
import { TextureManager, RenderTexture } from './textures';
...
export interface PassManagerOptions {
    ...
    /** Canvas colour format for the final pass target. Default the preferred canvas format. */
    canvasFormat?: GPUTextureFormat;
    /** RTT target format. Default `rgba16float`. */
    rttFormat?: GPUTextureFormat;
    /** Device sampled-texture-per-stage ceiling (assert + log, never throw). Default 16. */
...
export interface PassManager {
     ...
     */
    setComposition(ir: CompositionIR, uniformBindGroup: TgpuBindGroup | undefined, size: {
        width: number;
        height: number;
    }, mediaResources?: Record<string, RenderTexture | {
        texture: unknown;
    }>): void;
     ...
     */
    repaintRtt(): void;
    /** Resize all RTT textures to the new backing-buffer size. */
...
export declare function createPassManager(root: TgpuRoot, options: PassManagerOptions): PassManager;
