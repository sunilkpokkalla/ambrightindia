import { TgpuBindGroupLayout } from 'typegpu';
import { WgslStruct, AnyWgslData } from 'typegpu/data';
import { MaskType, NodeMetadata, BoundingBoxDeclaration, ComponentProps, PropConfig, CleanupCallback, RenderCallback, ResizeCallback, BlendMode } from '../types';
...
export interface GpuDataTextureOptions {
     ...
     * or `rgba16float` for a filterable multi-channel field texture (ChromaFlow's flow+density,
     * PixelThrow's 2D displacement) — half-encode each texel with kit `toHalfFloat` before upload.
     */
    format: 'r16float' | 'r32float' | 'rgba16float';
 ...
 * content into (`write`), and whose raw `GPUTexture` it can reach (`unwrap`, for DOMTexture's
...
export interface GpuFragmentParams {
     ...
     * the child generator at this link's looked-up UV (so `childNode` is already the colour at the right
     ...
     * (ImageTexture/Text/DOMTexture) creates a placeholder here at composition, uploads content via
    ...
    /** Raw device + root for DOMTexture / texture uploads. */
    gpu: {
        device: GPUDevice;
...
/** A fragment builder — runs at composition, returns the node's colour expression. */
...
export interface RegistryNode {
    ...
    /** Per-node DOM-capture host canvas (DOMTexture `capturesDOM`) — forwarded to `params.domCanvas`. */
    domCanvas?: HTMLCanvasElement;
}
