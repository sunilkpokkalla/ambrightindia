import { TgpuRoot, TgpuBindGroup, TgpuBindGroupLayout, TgpuTexture, TgpuFixedSampler } from 'typegpu';

/** Formats a data texture may take (float/byte, for compute/lookup inputs). */
export type DataTextureFormat = 'r32float' | 'r16float' | 'rg16float' | 'rg32float' | 'rgba8unorm' | 'rgba16float' | 'rgba32float';
type AnyImageSource = HTMLCanvasElement | HTMLImageElement | HTMLVideoElement | ImageBitmap | ImageData | OffscreenCanvas | VideoFrame;
...
export interface ManagedTexture {
    ...
    /** The raw GPUTexture (for DOMTexture's `queue.copyElementImageToTexture`). */
    unwrap(): GPUTexture;
    /**
...
export interface RenderTexture extends ManagedTexture {
    /** Resize by destroying and recreating the backing texture. */
...
export declare function createTextureManager(root: TgpuRoot): TextureManager;
export {};
