...
export declare const BLUR_DISK: [number, number][];
/**
...
export interface GlassEffectUniforms {
    ...
    scale: unknown;
    rotation: unknown;
    edgeSoftness: unknown;
    refraction: unknown;
    innerZoom: unknown;
    aberration: unknown;
    blur: unknown;
    tintColor: unknown;
    tintIntensity: unknown;
    tintPreserveLuminosity: unknown;
    lightAngle: unknown;
    highlight: unknown;
    highlightColor: unknown;
    highlightSoftness: unknown;
    fresnel: unknown;
    fresnelColor: unknown;
    fresnelSoftness: unknown;
    cutout: unknown;
    thickness: unknown;
}
 ...
 */
export declare const sdfSpaceUV: import('typegpu').TgpuFn<(center: d.Vec2f, scale: d.F32, rotation: d.F32, uv: d.Vec2f, aspect: d.F32) => d.Vec2f>;
/** `uv + vec2(ox, oy)` — builds a neighbour/trace sample UV from a base UV + scalar offsets. */
export declare const offsetUV: import('typegpu').TgpuFn<(uv: d.Vec2f, ox: d.F32, oy: d.F32) => d.Vec2f>;
/** Forward-difference gradient of the SDF (non-baked path): `((sdfX - sdf)/EPS, (sdfY - sdf)/EPS)`
 *  with EPS = 0.01. Pure. */
export declare const glassGradient: import('typegpu').TgpuFn<(sdfRaw: d.F32, sdfX: d.F32, sdfY: d.F32) => d.Vec2f>;
/**
 ...
 */
export declare const glassRefrStrength: import('typegpu').TgpuFn<(sdf: d.F32, thickness: d.F32, volumetric: d.F32) => d.F32>;
/** The refracted lens sample UV + the ±chromatic-aberration r/b UVs. Pure. */
export declare const GlassLensUVs: d.WgslStruct<{
    lensUV: d.Vec2f;
    rUV: d.Vec2f;
    bUV: d.Vec2f;
}>;
export declare const glassLensUVs: import('typegpu').TgpuFn<(uv: d.Vec2f, center: d.Vec2f, sdfRaw: d.F32, scale: d.F32, gradX: d.F32, gradY: d.F32, refraction: d.F32, innerZoom: d.F32, thickness: d.F32, aberration: d.F32, aspect: d.F32, volumetric: d.F32) => d.WgslStruct<{
    lensUV: d.Vec2f;
    rUV: d.Vec2f;
    bUV: d.Vec2f;
}>>;
/** One Vogel-disk blur tap offset in UV space: `(1/viewport) * (ox,oy) * blur*2`. Pure. */
export declare const glassBlurOffset: import('typegpu').TgpuFn<(viewportSize: d.Vec2f, blur: d.F32, ox: d.F32, oy: d.F32) => d.Vec2f>;
/**
 * Colour tint (mix-based, matches the Tint shader). Optional luminosity preservation rescales the
 * tinted colour back to the source luminance. `preserveLum` is 0/1. Pure — CPU-golden-testable
 * (the boolean choice is a `std.mix` with a 0/1 factor, not a vec `std.select`).
 */
export declare const glassTint: import('typegpu').TgpuFn<(rgb: d.Vec3f, tintColor: d.Vec3f, tintIntensity: d.F32, preserveLum: d.F32) => d.Vec3f>;
/** Directional border highlight ring (rb2): a thin 1px ring modulated by `normal·light`. Pure. */
export declare const glassBorderHighlight: import('typegpu').TgpuFn<(sdf: d.F32, edgeSoftness: d.F32, pxH: d.F32, gradX: d.F32, gradY: d.F32, lightAngle: d.F32, highlight: d.F32) => d.F32>;
/** Phong specular glint on the SDF normal (view dir = +z). Pure — CPU-golden-testable. */
export declare const glassSpecular: import('typegpu').TgpuFn<(gradX: d.F32, gradY: d.F32, lightAngle: d.F32, highlight: d.F32, highlightSoftness: d.F32, refrStrength: d.F32) => d.F32>;
...
export declare const glassFresnelRim: import('typegpu').TgpuFn<(sdf: d.F32, gradX: d.F32, gradY: d.F32, fresnel: d.F32, fresnelSoftness: d.F32, rb1: d.F32, volumetric: d.F32) => d.F32>;
/**
 ...
 */
export declare function applyGlassEffect(params: GpuFragmentParams, sdfSampler: (uv: Expr) => Expr, childTexture: KitTexture, blurredTexture?: KitTexture, options?: GlassEffectOptions): Expr;
