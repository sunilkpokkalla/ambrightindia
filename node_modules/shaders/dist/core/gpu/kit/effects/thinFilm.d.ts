 ...
 */
export declare const thinFilmShade: import('typegpu').TgpuFn<(sdfRaw: d.F32, scale: d.F32, gradX: d.F32, gradY: d.F32, edgeSoftness: d.F32, rimWidthUniform: d.F32, thickness: d.F32, dispersion: d.F32, hueShift: d.F32, lightAngle: d.F32, animTime: d.F32, volumetric: d.F32) => d.Vec2f>;
/**
 * IQ cosine palette → smooth iridescent spectrum (oil-slick / anodised metal). `t = phase`.
 * `cos((phase + k)·2π)·0.5 + 0.5` per channel with k = 0, 1/3, 2/3. Pure — CPU-golden-testable.
 */
export declare const thinFilmRainbow: import('typegpu').TgpuFn<(phase: d.F32) => d.Vec3f>;
/**
 * Custom 3-colour cycle parameters from the phase: returns `vec3(si, gate12, gate01)` where `si` is
 * the in-segment blend factor and `gate12`/`gate01` are WGSL `step` gates selecting the active
 * segment (A→B→C→A). Pure — CPU-golden-testable. The colours themselves are mixed in the builder
 * with the compile-time-selected `mixColorsVariants[mode]`.
 */
export declare const thinFilmCustomParams: import('typegpu').TgpuFn<(phase: d.F32) => d.Vec3f>;
/**
 * Final composite: blend a plain white rim ↔ the spectral colour by `saturation`, scale by the rim
 ...
 */
export declare const thinFilmCompose: import('typegpu').TgpuFn<(baseColor: d.Vec3f, rim: d.F32, intensity: d.F32, saturation: d.F32) => d.Vec4f>;
/**
 * Apply the iridescent thin-film rim effect. Composition-time Expr builder — see the file header
 * for the notes. A transparent-interior generator (no child): RGB is the rim colour, alpha is the
 ...
 */
export declare function applyThinFilmEffect(params: GpuFragmentParams, sdfSampler: (uv: Expr) => Expr, options?: ThinFilmEffectOptions): Expr;
