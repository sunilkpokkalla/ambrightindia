import { TgpuBindGroupLayout, TgpuFragmentFn } from 'typegpu';
import { BlendMode } from './kit/blend';
import { MaskType } from './kit/mask';
import { ToneMappingMode } from './kit/tonemap';
import { ComputeStep } from './compute';
import { Expr, GpuMediaTexture, RegistryView } from './contract';

export declare const SHARED_SAMPLER_LAYOUT: TgpuBindGroupLayout<{
    linearClamp: {
        sampler: "filtering";
    };
    nearestClamp: {
...
export declare const BIND_GROUPS: {
    readonly uniforms: 0;
    readonly textures: 1;
    readonly samplers: 2;
    readonly external: 3;
...
export declare function expr(wgsl: string): Expr;
/** A WGSL float literal Expr (decimal-safe). */
export declare function floatE(n: number): Expr;
/** A call to a `tgpu.fn` external: `<name>(arg0, arg1, …)`. Dedupes the fn by identity. */
export declare function call(fn: unknown, hint: string, args: Expr[]): Expr;
/** `vec4f(rgb, a)` or `vec4f(x, y, z, w)`. */
export declare function vec4(...parts: (Expr | number)[]): Expr;
/** `mix(a, b, t)` — WGSL built-in, no external. */
export declare function mixExpr(a: Expr, b: Expr, t: Expr | number): Expr;
/**
 ...
 */
export declare function arrayExpr(elemType: string, elems: Expr[]): Expr;
/** Transparent-black `vec4f(0.0, 0.0, 0.0, 0.0)`. */
export declare const ZERO: Expr;
/** `vec4f(1.0, 1.0, 1.0, 0.0)` — the first-child blend base. */
export declare const WHITE0: Expr;
/** `blendModes[mode](base, overlay, opacity)`. */
export declare function applyBlend(base: Expr, overlay: Expr, mode: BlendMode, opacity: Expr): Expr;
/** `maskFunctions[type](target, mask)`. */
export declare function applyMaskFn(target: Expr, mask: Expr, type: MaskType): Expr;
...
export interface ComposeOptions {
     ...
     * the static handle (correct for GPU-free tests / static maps). Not part of the structural hash.
     */
 ...
 */
export declare function composeNodeTree(registry: RegistryView, options?: ComposeOptions): CompositionIR;
/**
 ...
 */
export declare function collectStructuralHashInputs(registry: RegistryView, options?: ComposeOptions): string[];
