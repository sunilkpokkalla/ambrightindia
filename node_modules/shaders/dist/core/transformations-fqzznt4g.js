...
function clamp(min, val, max$1) {
	return Math.max(Math.min(max$1, val), min);
}
	...
	inGamut(coords, { epsilon = ε$7 } = {}) {
		...
		return coords.every((c$1, i) => {
			...
			if (meta.type !== "angle" && meta.range) {
				...
				let [min, max$1] = meta.range;
				return (min === void 0 || c$1 >= min - epsilon) && (max$1 === void 0 || c$1 <= max$1 + epsilon);
			}
			return true;
		});
	}
	...
	if (method === "css") spaceColor = toGamutCSS(color, { space });
		...
		if (method === "clip" || !inGamut(spaceColor, space, { epsilon: 0 })) {
			...
			spaceColor.coords = spaceColor.coords.map((c$1, i) => {
				let [min, max$1] = bounds[i];
				if (min !== void 0) c$1 = Math.max(min, c$1);
				if (max$1 !== void 0) c$1 = Math.min(c$1, max$1);
				return c$1;
			});
		}
	}
...
function toGamutCSS(origin, { space } = {}) {
	...
	function clip(_color) {
		...
		destColor.coords = destColor.coords.map((coord, index) => {
			if ("range" in spaceCoords[index]) {
				const [min$1, max$2] = spaceCoords[index].range;
				return clamp(min$1, coord, max$2);
			}
			return coord;
		});
		return destColor;
	...
	let min = 0;
	let max$1 = origin_OKLCH.coords[1];
...
function mix(c1$3, c2$3, p$1 = .5, o = {}) {
...
var HSL = new ColorSpace({
	...
	fromBase: (rgb) => {
		...
		let d$1 = max$1 - min;
		if (d$1 !== 0) {
			s = l === 0 || l === 1 ? 0 : (max$1 - l) / Math.min(l, 1 - l);
			switch (max$1) {
				case r:
...
var colorSpaceMode = "p3-linear";
const setColorSpaceMode = (mode) => {
	colorSpaceMode = mode;
};
...
const transformColor = (value) => parseColorChannels(value);
...
const transformStrokePosition = (value) => {
	return {
		"outside": 0,
		"center": 1,
		"inside": 2
...
export { transformColor as a, transformPosition as c, transformBoolean as i, transformStrokePosition as l, setColorSpaceMode as n, transformColorSpace as o, transformAngle as r, transformEdges as s, colorSpaceOptions as t, Color as u };
