          ...
          <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3 sm:gap-6">
            {posts.map((p) => (
              <Link
                key={p.path}
                to={p.path}
                className="card-lift group flex flex-col gap-3 overflow-hidden rounded-2xl border border-gray-200 bg-white transition-shadow duration-300 hover:shadow-[0_4px_16px_rgba(0,0,0,0.08)]"
              >
                {p.image && (
                  <img
                    src={p.image}
